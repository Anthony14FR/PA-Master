...
import { setAccessToken, setRefreshToken, getCookie } from '@kennelo/core/storage/utils/cookies.server';
import { domainService } from '@kennelo/core/api/services/domain.service';
import { getTargetDomainFromCountry, buildRedirectHost } from '@kennelo/i18n/lib/i18n';
import { AUTH_NAMESPACE, PAGES, LOGIN_CONFIG } from '@kennelo/core/auth/configs/access-control.config';
...
class RedirectService {
    ...
    _buildCleanUrl(request, path) {
        ...
        const cleanHost = port ? cleanHostname + ':' + port : cleanHostname;
        return new URL(protocol + '//' + cleanHost + path);
    }
}
